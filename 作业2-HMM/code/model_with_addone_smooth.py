...
#	print(emission)
	for i in range(len_tag):
		...
		emission_prob[i] = 100*(1+emission[i])/(2*emission_sum)
#	print(emission_prob)
...
def estimateTransitionProb(tagdict, sequences_tags, tags2ids):
	...
	trans_sum_sum = np.sum(trans_sum)
	for i in range(len_tag):
		if trans_sum[i]==0:
			for j in range(len_tag):
				transition_prob[i][j] = 1/trans_sum_sum
		for j in range(len_tag):
			#transition_prob[i][j] = -1.0 if (transition[i][j]==0) else 100* transition[i][j]/trans_sum[i]
			transition_prob[i][j] = 100*(1+ transition[i][j])/(trans_sum_sum+trans_sum[i])
